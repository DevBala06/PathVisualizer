...
import { usePathfindingStore } from "../hooks/usePathfindingConfig";

const Card = ({ children }: { children: ReactElement }) => {
  const { tabs, addTab, activeTab } = useTabStore();
  const [isDisabled, setIsDisabled] = useState(false);
  const { isOpen, setIsOpen, isVisualizationRunningRef } = usePathfinding();
  const { speed } = useSpeed();
  const { gridConfigs, setGrid, setIsGraphVisualized, initializeConfig } =
    usePathfindingStore();
  const router = useRouter();

  // Compute activeTabId even if filteredTabs is empty.
  const filteredTabs = tabs.filter((tab) => tab.id === activeTab);
  console.log(filteredTabs);
  const activeTabId: string = filteredTabs.length > 0 ? filteredTabs[0].id.toString() : "1";
  
  // Always call useEffect, then conditionally render later.
  useEffect(() => {
    if (activeTabId && !gridConfigs[activeTabId]) {
      initializeConfig(activeTabId);
    }
  }, [activeTabId, gridConfigs, initializeConfig]);

  // After hooks, conditionally render the loading UI.
  console.log(activeTabId)
  if (!activeTabId) return <div>Loading...</div>;
  const config = gridConfigs[activeTabId];
  console.log(config)
  if (!config) return <div>Loading...</div>;

  const { algorithm, grid, isGraphVisualized, startTile, endTile } = config;

  const handlerRunVisualizer = () => {
    if (isGraphVisualized) {
      setIsGraphVisualized(activeTabId, false);
    ...
    setTimeout(() => {
      setGrid(activeTabId, grid.slice());
      setIsGraphVisualized(activeTabId, true);
      setIsDisabled(false);
        ...
        <div className="w-screen h-screen bg-[#141414] flex flex-col">
          ...
          <div className="flex w-full items-center justify-between border-b border-gray-700 px-3 py-1 gap-2">
            <div className="flex gap-1 items-center">
              <div
                className="w-2.5 h-2.5 bg-red-500 rounded-full shadow-inner cursor-pointer"
            ...
            <div className="relative flex items-center border border-zinc-600 rounded-md px-52 py-1 text-sm text-gray-300">
              <span className="pl-6 flex gap-1 items-baseline text-sm font-semibold">
                <FaLock />pathvisualizer.io
              </span>
